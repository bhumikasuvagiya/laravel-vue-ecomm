...
export const useCartStore = defineStore({
  ...
  getters: {
    ...
    total(): number {
      ...
      return Object.keys(this.contents).reduce((acc, id) => {
        return acc +  this.contents[id].quantity * products.items[id].amount;
      }, 0);
    },
    ...
    formattedCart(): CartPreview[] {
      ...
      return Object.keys(this.contents).map((productId) => {
        ...
        return {
          ...
          image: products.items[purchase.productId].product_image,
          title: products.items[purchase.productId].product_name,
          quantity: purchase.quantity,
          cost: purchase.quantity * products.items[purchase.productId].amount,
        };
      });
    },
  },
